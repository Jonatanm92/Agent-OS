/*
 * render-preview.mjs — renders the cmi-* Liquid sections into standalone HTML.
 *
 * WHY THIS EXISTS
...
const ONE_NIGHT = makeProduct({
  handle: 'one-night-personalized-concert-memory-issue',
  title: 'One Night — Personalized Concert Memory Issue',
  price: 34900,
  tags: ['cmi', 'cmi-tier-one-night'],
});

const FULL_ISSUE = makeProduct({
  handle: 'full-issue-personalized-concert-memory-edition',
...
const allProducts = {
  [ONE_NIGHT.handle]: ONE_NIGHT,
  [FULL_ISSUE.handle]: FULL_ISSUE,
};
...
<title>${title}</title>
<style>body{margin:0;padding:0;background:#101014;font-family:system-ui,sans-serif;}</style>
</head>
<body>
${chunks.join('\n')}
</body>
...
const jobs = [
  ['product.concert-memory.json', 'product-one-night.html',
   { product: ONE_NIGHT, title: 'One Night — Personalized Concert Memory Issue' }],
  ['product.concert-memory.json', 'product-full-issue.html',
   { product: FULL_ISSUE, title: 'Full Issue — Personalized Concert Memory Edition' }],
  ['index.json', 'home.html', { product: null, title: 'JM Store — Personalized concert memories' }],
];
