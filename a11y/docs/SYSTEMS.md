...
| 2 | Site discovery | Built | `discovery/JourneyDiscovery.ts`, `LinkClassifier.ts`, `Robots.ts` |
| 3 | Audit engine | Built | `audit/` — axe-core, keyboard, focus, forms, structure, dialogs, reflow |
| 4 | Finding normalization | Built | `findings/Normalize.ts`, `Dedupe.ts`, `RuleCatalog.ts`, `WcagMap.ts` |
