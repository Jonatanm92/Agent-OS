...
node radar.mjs analyze --country us                                # writes data/report.md + report.json
```
...
**Limits:** Apple's RSS returns at most 500 recent reviews per app, and some apps return none (ReciMe and NYT Cooking did in the 2026-09-23 run). Theme tagging is keyword-based, so read the quotes (`report.json → quotes`) before deciding. Google Play isn't covered yet.
